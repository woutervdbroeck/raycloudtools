...
def calculate_volume(tree_mesh):
    tree_arr = np.array(tree_mesh).reshape((-1,6))
...
def read_tree_mesh(file_path):
    with open(file_path,'r') as f:
        for i, line in enumerate(f):
            if i > 1:
                tree_mesh = line.strip().split(',')
    return tree_mesh
...
def process_raycloud_files(input_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('_raycloud_trees.txt'):
            ...
            tree_mesh = read_tree_mesh(file_path)

            # Calculate volume according to some function
            volume = calculate_volume(tree_mesh)
